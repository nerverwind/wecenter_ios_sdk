...
class topic extends AWS_CONTROLLER
{
    ...
    public function get_parent_topics_action(){
        $ret = $this->model('topic')->get_parent_topics();
    ...
    //获取父话题 - add by wuzhongxin
    public function parent_topics_action(){
        ...
        $topic_info = $this->model('topic')->get_parent_topics_page($topic_id, $page, $per_page);
        if(!$topic_info){
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('没有符合条件的话题')));
        }
        ...
        H::ajax_json_output(AWS_APP::RSM(array(
            'total_rows' => count($topic_info),
            'rows' => array_values($topic_info)
        ), 1, null));

    }

    //获取子话题 - add by wuzhongxin
    public function child_topics_action(){
        $topic_id = intval($_POST['topic_id']);
        $page = intval($_POST['page']);
        $per_page = get_setting('contents_per_page');
        ...
        $topic_info = $this->model('topic')->get_child_topics_page($topic_id, $page, $per_page);
        if(!$topic_info){
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('没有符合条件的话题')));
        }
    ...
    public function hot_topics_action(){
        ...
        if (!$topic_info = $this->model('topic')->get_hot_topics_page($day, $page, $per_page))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('未找到符合条件的话题')));
        }
