...
class topic_class extends AWS_MODEL
{
    ...
    /**
     * 获取当前热门话题 - add by wuzhongxin
     * 约定热门话题的条数应为: $limit * $page
     */
    public function get_hot_topics_page($section = null, $page, $limit){
        $where = array();
        switch ($section)
        {
            default:
                return $this->fetch_page('topic', implode(' AND ', $where), 'discuss_count DESC', $page, $limit);
                break;
            case 'week':
                $where[] = 'discuss_count_update > ' . (time() - 604801);

                return $this->fetch_page('topic', implode(' AND ', $where), 'discuss_count_last_week DESC', $page, $limit);
                break;

            case 'month':
                $where[] = 'discuss_count_update > ' . (time() - 2592001);

                return $this->fetch_page('topic', implode(' AND ', $where), 'discuss_count_last_month DESC', $page, $limit);
                break;
        }
    }
    ...
    //add by wuzhongxin
    public function get_parent_topics_page($topic_id, $page, $limit = 10){
        if($topic_id){
            $parent_topic_list_query = $this->fetch_page('topic', 'is_parent = 1 AND parent_id = ' . intval($topic_id), 'topic_title ASC', $page, $limit);
        }
        else{
            $parent_topic_list_query = $this->fetch_page('topic', 'is_parent = 1', 'topic_title ASC', $page, $limit);
        }
    ...
    //add by wuzhongxin
    public function get_child_topics_page($topic_id, $page, $limit = 10){
        $child_topics_list_query =  $this->fetch_page('topic', 'parent_id = ' . intval($topic_id), 'add_time DESC', $page, $limit);

        if (!$child_topics_list_query)
        {
            return false;
        }

        foreach ($child_topics_list_query AS $child_topic_info)
        {
            if (!$child_topic_info['url_token'])
            {
                $child_topic_info['url_token'] = urlencode($child_topic_info['topic_title']);
            }

            $child_topic_list[$child_topic_info['topic_id']] = $child_topic_info;
        }
